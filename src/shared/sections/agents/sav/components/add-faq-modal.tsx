...
export interface AddFaqModalProps {
  open: boolean;
  onClose: () => void;
  onAdd: (category: string, question: string, answer: string) => Promise<void> | void;
  onEdit?: (id: string, category: string, question: string, answer: string) => Promise<void> | void;
  categories: string[];
  faqToEdit?: FAQ | null;
  containerId?: string;
}
  ...
  const handleAdd = async () => {
    ...
    try {
      if (isEditMode && faqToEdit && onEdit) {
        await onEdit(faqToEdit.id, category, question, answer);
      } else {
        await onAdd(category, question, answer);
      }
      handleReset();
    } catch (err: any) {
      console.error('Add/Edit FAQ error:', err);
      ...
      <DialogActions sx={{ p: 4, pt: 2, flexDirection: 'column', gap: 2, alignItems: 'center' }}>
        ...
        >
          <ConditionalComponent
            isValid={submitting}
            defaultComponent={isEditMode ? 'Modifier la FAQ' : 'Ajouter la FAQ'}
          >
            <CircularProgress size={20} sx={{ color: '#3C1C69' }} />
          </ConditionalComponent>
        </Button>
